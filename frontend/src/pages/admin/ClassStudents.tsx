...
import { useAppStore, StudentData, SessionData, ClassData } from '../../store/appStore';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Dialog, DialogContent, DialogHeader, DialogTitle } from '@/components/ui/dialog';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from '@/components/ui/dropdown-menu';
import { Plus, MoreVertical, ArrowLeft, User } from 'lucide-react';
...
export default function ClassStudents({ session, classData }: Props) {
  const { students, addStudent, updateStudent, deleteStudent, passStudent, sessions } = useAppStore();
  const classStudents = students.filter((s) => s.classId === classData.id && s.sessionId === session.id)
  ...
  const handleSubmit = () => {
    ...
    if (editingId) {
      updateStudent(editingId, { ...form, classId: classData.id, sessionId: session.id });
      toast.success('Student updated');
    } else {
      addStudent({ ...form, classId: classData.id, sessionId: session.id });
  ...
  const handlePass = () => {
    ...
    setShowPass(null); setPassSession(''); setPassClass('');
  };
      ...
      <div className="p-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 space-y-3">
          ...
          {[
            ...
            ['Father Occupation', viewingStudent.fatherOccupation],
            ['Mother Occupation', viewingStudent.motherOccupation],
            ['Guardian Info', viewingStudent.guardianInfo],
            ['Username', viewingStudent.username],
  ...
  return (
    <div className="p-4">
      <div className="flex items-center justify-between mb-4">
        <span className="text-sm font-medium text-gray-600">{classStudents.length} student{classStudents.length !== 1 ? 's' : ''}</span>
        <Button onClick={() => { resetForm(); setShowForm(true); }} size="sm" className="bg-orange-500 hover:bg-orange-600 rounded-full w-8 h-8 p-0">
          <Plus className="w-4 h-4" />
        </Button>
      </div>

      {classStudents.length === 0 ? (
        <div className="text-center py-12 text-gray-400 text-sm">No students yet</div>
      ) : (
        <div className="space-y-2">
          {classStudents.map((s) => (
            <div key={s.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-3 flex items-center justify-between">
              <div>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem className="text-red-500" onClick={() => { deleteStudent(s.id); toast.success('Student deleted'); }}>Delete</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
          ))}
        ...
        <DialogContent className="max-w-sm max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            {[
              ...
              { label: 'Admission No *', key: 'admissionNo', placeholder: 'Admission number' },
              { label: 'Aadhaar No', key: 'aadhaarNo', placeholder: 'Aadhaar number' },
              { label: 'Date of Birth', key: 'dateOfBirth', placeholder: 'DD/MM/YYYY', type: 'date' },
              { label: 'Date of Joining', key: 'dateOfJoining', placeholder: 'DD/MM/YYYY', type: 'date' },
              { label: 'Address', key: 'address', placeholder: 'Address' },
              { label: 'Father Name', key: 'fatherName', placeholder: "Father's name" },
              { label: 'Mother Name', key: 'motherName', placeholder: "Mother's name" },
              { label: 'Father Mobile', key: 'fatherMobile', placeholder: 'Father mobile no.' },
              { label: 'Mother Mobile', key: 'motherMobile', placeholder: 'Mother mobile no.' },
              { label: 'Father Occupation', key: 'fatherOccupation', placeholder: 'Optional' },
              { label: 'Mother Occupation', key: 'motherOccupation', placeholder: 'Optional' },
              { label: 'Guardian Info', key: 'guardianInfo', placeholder: 'Optional' },
              { label: 'Username', key: 'username', placeholder: 'Login username' },
              { label: 'Password', key: 'password', placeholder: 'Login password' },
              { label: 'Code', key: 'code', placeholder: 'School code' },
      ...
      <Dialog open={!!showPass} onOpenChange={(o) => { if (!o) setShowPass(null); }}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-gray-600">Transfer <strong>{showPass?.name}</strong> to:</p>
          <div className="space-y-3">
            <div>
